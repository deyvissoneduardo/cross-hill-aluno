...
export function SuccessStep() {
  ...
  const [configuracao, setConfiguracao] = useState<ConfiguracaoSucesso>(FALLBACK_SUCESSO)
  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col gap-4" aria-label="Carregando confirmação da solicitação">
  ...
  return (
    <section className="flex flex-col gap-5" aria-labelledby="success-heading">
      <div
        aria-hidden="true"
        className="flex size-12 items-center justify-center rounded-full border border-[var(--success)]/40 bg-[var(--success)]/10 text-xl font-semibold text-[var(--success)]"
      >
        ✓
      </div>

      <div className="flex flex-col gap-2">
        <h1 id="success-heading" className="text-2xl font-semibold text-[var(--foreground)]">
          {configuracao.titulo}
        </h1>
        <p className="text-sm leading-6 text-[var(--text-muted)]">{configuracao.descricao}</p>
      </div>
...
function normalizarConfiguracao(configuracao: ConfiguracaoSucesso): ConfiguracaoSucesso {
